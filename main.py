...
@click.option(
    ...
    "-i",
    "input_dir",
    help="Directorio de entrada raíz con subcarpetas que contengan carpetas TIFF",
)
@click.option(
...
@click.option(
    "--verbose", "-v", is_flag=True, help="Modo verbose con más información en pantalla"
)
def main(
    input_dir: str,
    output_dir: str,
...
def _show_configuration(
    ...
    output_manager.section("CONFIGURACIÓN")
    output_manager.separator()
    ...
    output_manager.format_info(
        "🔄 Formatos a convertir", formats or "Todos los habilitados"
    )
    output_manager.format_info("👥 Workers paralelos", max_workers or "Automático")
    output_manager.info(
        "📁 El sistema buscará carpetas TIFF en subcarpetas recursivamente"
    )
...
def _show_detailed_results(result: dict) -> None:
    ...
    output_manager.format_info(
        "📁 Subcarpetas procesadas",
        f"{result.get('subfolders_successful', 0)}/{result.get('subfolders_processed', 0)}",
    )
    output_manager.format_info(
        "📄 Archivos totales", result.get("total_files_processed", 0)
    )
    output_manager.format_info(
        "✅ Conversiones exitosas", result.get("total_conversions_successful", 0)
    )
    output_manager.format_info(
        "❌ Conversiones fallidas", result.get("total_conversions_failed", 0)
    )
    output_manager.format_info(
        "⏱️  Tiempo total", f"{result.get('time_elapsed', 0):.2f} segundos"
    )
...
def _show_subfolder_summary(result: dict) -> None:
    ...
    for subfolder_name, subfolder_summary in summary.items():
        ...
        if subfolder_summary.get("failed", 0) > 0:
            output_manager.warning(
                f"   ⚠️  {subfolder_summary.get('failed', 0)} conversiones fallidas"
            )
