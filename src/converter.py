        ...
        try:
            ...
            if formats:
                ...
                if not formats:
                    return {
                        'success': False,
                        'error': f'No hay conversores disponibles para los formatos especificados: {formats}',
                        'files_processed': 0,
                        'conversions_successful': 0,
                        'conversions_failed': 0,
                        'time_elapsed': 0
                ...
                for tiff_file in tiff_files:
                    for format_name in formats:
                        ...
                        if hasattr(converter, 'get_output_filename'):
                            output_path = converter.get_output_filename(tiff_file, file_processor.output_dir)
                        else:
    ...
    def _print_summary(self, result: Dict[str, Any]) -> None:
        ...
        if result['success']:
            ...
            if 'jpg_400' in result['formats_processed']:
                print(f"🖼️  JPG 400 DPI: Alta resolución para impresión")
            if 'jpg_200' in result['formats_processed']:
                print(f"🖼️  JPG 200 DPI: Resolución media para web")
            if 'pdf_easyocr' in result['formats_processed']:
                print(f"📄 PDF con EasyOCR: Texto buscable y seleccionable")
        else:
...
def main():
    ...
    try:
        ...
        print(f"   python main.py --input 'entrada' --output 'salida'")
