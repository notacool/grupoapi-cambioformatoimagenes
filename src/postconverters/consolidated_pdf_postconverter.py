...
class ConsolidatedPDFPostconverter(BasePostConverter):
    ...
    def process(self, conversion_result: Dict[str, Any], output_dir: Path) -> bool:
        ...
        try:
            if not self.enabled:
                output_manager.info("🔄 Consolidated PDF Postconverter deshabilitado, saltando...")
                return True
            
            subfolder_name = conversion_result.get("subfolder")
            if not subfolder_name:
                output_manager.error("❌ No se encontró nombre de subcarpeta en el resultado")
                return False
            
            # Obtener archivos TIFF de la subcarpeta
            tiff_files = self._get_tiff_files_from_subfolder(subfolder_name, output_dir)
            if not tiff_files:
                output_manager.info(f"ℹ️ No se encontraron archivos TIFF en {subfolder_name}")
                return True
            
            # Ordenar archivos alfabéticamente
            tiff_files.sort(key=lambda x: x.name.lower())
            
            output_manager.info(f"🔄 Consolidando {len(tiff_files)} archivos TIFF de {subfolder_name}")
            ...
            if success:
                output_manager.success(f"✅ PDF consolidado creado exitosamente para {subfolder_name}")
            else:
                output_manager.error(f"❌ Error al crear PDF consolidado para {subfolder_name}")
            
            return success
            
        except Exception as e:
            error_msg = f"Error en ConsolidatedPDFPostconverter para {subfolder_name}: {str(e)}"
    ...
    def _get_tiff_files_from_subfolder(self, subfolder_name: str, output_dir: Path) -> List[Path]:
        """
        ...
        try:
            ...
            output_manager.error(f"❌ Error obteniendo archivos TIFF de {subfolder_name}: {str(e)}")
            return []
    ...
    def _consolidate_tiff_to_pdf(
        ...
        try:
            ...
            if files_per_pdf >= total_files:
                ...
                # Múltiples PDFs
                return self._create_multiple_pdfs(tiff_files, output_dir, subfolder_name, files_per_pdf)
    ...
    def _calculate_files_per_pdf(self, tiff_files: List[Path]) -> int:
        ...
        try:
            ...
            output_manager.info(f"ℹ️ Estimación: {files_per_pdf} archivos por PDF (máx {self.max_size_mb}MB)")
    ...
    def _create_single_pdf(
        ...
        try:
            ...
            if success:
                output_manager.success(f"✅ PDF consolidado creado: {subfolder_name}_consolidated.pdf")
    ...
    def _create_multiple_pdfs(
        ...
        try:
            ...
            for i in range(0, len(tiff_files), files_per_pdf):
                ...
                if pdf_files:
                    ...
                    if self._merge_pdfs(pdf_files, output_file):
                        success_count += 1
                        output_manager.success(f"✅ PDF {pdf_number}/{total_pdfs} creado: {output_file.name}")
            ...
            output_manager.success(f"✅ {success_count}/{total_pdfs} PDFs consolidados creados exitosamente")
    ...
    def _convert_tiff_to_pdf(self, tiff_file: Path, output_dir: Path) -> Optional[Path]:
        ...
        try:
            ...
            success = self.pdf_converter.convert_file(str(tiff_file), str(temp_pdf_path))
    ...
    def _merge_pdfs(self, pdf_files: List[Path], output_file: Path) -> bool:
        ...
        try:
            ...
            try:
                ...
                output_manager.warning("⚠️ PyPDF2 no disponible, usando método alternativo")
